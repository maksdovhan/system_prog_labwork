...
void myWorkSSE(HWND hWnd)
{
    ...
    MessageBoxA(hWnd, text.c_str(), "Скалярний добуток SSE", MB_OK);
...
void myWorkCPP(HWND hWnd)
{
    ...
    MessageBoxA(hWnd, text.c_str(), "Скалярний добуток С++", MB_OK);

    std::string text3 = std::to_string(ten);
    MessageBoxA(hWnd, text3.c_str(), "Час виконання С++", MB_OK);
}
